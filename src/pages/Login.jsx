...
const Login = () => {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      const createProfileFromPendingSignup = async () => {
        const pendingSignup = localStorage.getItem('pendingSignup');
        if (!pendingSignup) return null;

        const signupData = JSON.parse(pendingSignup);
        if (signupData.id !== data.user.id) return null;
        ...
        if (insertError) {
          // Ignore duplicate profile insert if it already exists
          if (insertError.code !== 'PGRST116' && insertError.status !== 409) {
      ...
      while (retryCount < maxRetries) {
        const { profile: profileData } = await getExistingProfile();
        ...
        if (retryCount === maxRetries - 1) {
          profile = await createProfileFromPendingSignup();
          break;
        }
      ...
      if (!profile) {
        throw new Error('Unable to fetch or create user profile after multiple attempts. Please ensure your account has a profile row in the database.');
      }
